...
const assertArraysEqual  = function(firstList, secondList, message) {
  const firstListLength = firstList.length;
  const secondListLength = secondList.length;
  let result = true;
...
const assertObjectsEqual =  function(expected, actual, message) {
  ...
  const result = (assertArraysEqual(actualKeys, expectedKeys)) && (assertArraysEqual(actualValues, expectedValues))
  const icon = result === true ?  '✅ ' : '❌ ';
